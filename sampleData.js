...
const LOCATIONS = [
  ...
  { city: 'Dahod', spots: [
    { name: 'Dahod Bus Stand', lat: 22.8340, lng: 74.2530 },
    { name: 'Station Road Dahod', lat: 22.8380, lng: 74.2480 },
  ]},
];
...
export const searchVehicle = (plate, cameras, detections) => {
  ...
  return detections
    .filter(d => d.plate && d.plate.replace(/\s/g, '').toUpperCase().includes(norm))
    .map(d => {
      const cam = (cameras || []).find(c => c.id === d.cameraId);
      return { 
        ...d, 
        camera: cam || { id: d.cameraId, name: d.cameraId, city: 'Gujarat' }, 
        lat: cam?.lat || 23.0225 + (Math.sin(d.id || 1) * 0.15), 
        lng: cam?.lng || 72.5714 + (Math.cos(d.id || 1) * 0.15), 
        cameraName: cam?.name || d.cameraId, 
        city: cam?.city || 'Gujarat' 
      };
    })
